...
export default function Row({ r }: { r: FullRequest }) {
  return (
    <tr key={r.id}>
      ...
      <td className="border px-3 py-2">{REQUEST_STATUS_LABEL[r.status]}</td>
        ...
        {r.status === "PENDING" && (
          <div className="flex gap-1 justify-evenly">
            <form action={approve}>
              ...
              <button className="btn-approve cursor-pointer">承認</button>
            </form>
            <form action={reject}>
              <input type="hidden" name="id" value={r.id} />
              <button className="btn-reject cursor-pointer">却下</button>
            </form>
          </div>
        )}
      </td>
